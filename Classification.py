...
def plots(images, figsize=(20, 30), rows=10, interp=False, titles=None):
    if type(images[0]) is np.ndarray:
        images = np.arr(images).astype(np.uint8)
        if (images.shape[-1] != 3):
            images = images.transpose((0, 2, 3, 1))
    f = plt.figure(figsize=figsize)
    cols = 10  # len(images)//rows if len(images) % 2 == 0 else len(images)//rows + 1
    for i in range(0, 50):
...
def malware_model():
    ...
    Malware_model.add(Dense(number_classes, activation='softmax'))
...
def main():
    ...
    for counter, label in enumerate(files):
        ...
        # plt.imshow(convertAndSave(np.arr(arr),label))
        del arr
        f.close()
